...
async function CreateBlock(info, row) {
	...
	if (info.type == "present") {
		card.className =
			"panel panel-primary border border-5 border-success rounded-3";
	}
	if (info.type == "not-present") {
		card.className =
			"panel panel-primary border border-5 border-primary rounded-3";
	}
	if (info.type == "past") {
		card.className =
			"panel panel-primary border border-5 border-white rounded-3";
	}
